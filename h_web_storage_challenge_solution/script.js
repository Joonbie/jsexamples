...
async function fetchColorsList() {
  ...
  // purposely broke the link to simulate error handling
  const apiEndpoint = "abc123https://reqres.in/api/unknown";
...
btnLoadColors.addEventListener("click", () => {
  if (!listColors.innerHTML) fetchColorsList();
  else alert("Colors already loaded.");
});
...
async function ftnFetch(endpoint, page = 1) {
  ...
  try {
    ...
    //console.log is for checking errors. You can remove the .message to see all the errors on your console.
    console.log(error.message);
    listColors.innerHTML =
      "<span class='p-2'> Unable to load colours. Please visit us later. <span>";
  }
}
